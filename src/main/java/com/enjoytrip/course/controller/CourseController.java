...
public class CourseController {
    ...
    @PostMapping("")
    public ResponseEntity<?> makeCourse(@RequestBody CourseMakeRequest makeRequest,
        @AuthenticationPrincipal SessionUser sessionUser) {
        System.out.println("makeRequest = " + makeRequest);
        if (makeRequest.getAttractions().size() == 0) {
            return JsonResponse.fail("Attraction 을 최소한 한 개 이상 선택해주세요.", 400);
        }
        courseService.makeCourse(makeRequest, sessionUser.getId());
    ...
    @PostMapping("/comment")
    public ResponseEntity<?> commentAdd(@RequestBody CourseComment courseComment,
        @AuthenticationPrincipal SessionUser sessionUser) {
